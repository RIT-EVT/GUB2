...
void IRAM_ATTR CANDriverISRHandler(void *arg) {
    ...
    // If we successfully queued the new event, we need to tell the scheduler that there is now a high priority task that needs to run
    // Otherwise the scheduler will not begin running the CANDriverTask until the next SysTick occurs
    // because the scheduler cannot see that there is a new item in the queue as it was added from interrupt context
    if (xResult != pdFAIL) {
...
void printCANMessage(CANMessage_t const *msg) {
    printf(LOG_ANSI_COLOR_BOLD(LOG_ANSI_COLOR_RED) "Message on bus #%u ID: 0x%08" PRIx32 " with %d bytes of data: ",
           msg->bus,
           msg->ID, msg->DLC);
...
void printCANDriverState() {
    ...
    for (int i = 0; i < CAN_BUS_COUNT; i++) {
        ...
        printf("\t| %3d | %7lu | %s%8lu" LOG_RESET_COLOR " | %s%8lu" LOG_RESET_COLOR " | %s%7lu" LOG_RESET_COLOR
               ...
               busStats.receiveBufferFullCount,
               busStats.communicationErrorCount ? LOG_ANSI_COLOR_BOLD(LOG_ANSI_COLOR_RED) : "",
